...
class TargetKeeperPreprocessor(Preprocessor):
    """Used to remove a set of target volumes.
    """

    def __init__(self, targets):
        self.unwanted_targets = targets
    
    def __call__(self, images, targets):
        return images, np.delete(targets, self.unwanted_targets, axis=-1)
    
    def output_targets(self, input_targets):
        try:
            return input_targets - len(self.unwanted_targets)
        except TypeError:
            return input_targets - 1
